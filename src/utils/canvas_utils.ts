...
export function drawHover(context: CanvasRenderingContext2D, data: PlainObject, settings: PlainObject) {
    ...
    const label = data.attributes.label === 'Person' ? data.attributes.name : data.attributes.title;
    const subLabel = ""
    const clusterLabel = data.clusterLabel != '' ? data.clusterLabel : data.attributes.name;
